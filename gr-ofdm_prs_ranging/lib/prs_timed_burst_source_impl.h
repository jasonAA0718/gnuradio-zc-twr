...
namespace gr {
  namespace ofdm_prs_ranging {
    ...
    class prs_timed_burst_source_impl : public prs_timed_burst_source
    {
     ...
     public:
      prs_timed_burst_source_impl(double samp_rate, int fft_len, int cp_len, int active_bins, int prs_symbols, int preamble_len, int preamble_repeats, int coarse_sync_len, int zero_guard_len, int tail_guard_len, double tx_lead_time, double burst_period, float tx_amp, uint32_t seed, int pings_per_trigger, bool attach_tx_time);
