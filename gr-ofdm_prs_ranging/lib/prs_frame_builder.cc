...
{
    const int root = 25;
    for (int n = 0; n < cfg.coarse_sync_len; ++n) {
        const double phase =
            -pi * static_cast<double>(root) * n * (n + 1) /
            static_cast<double>(cfg.coarse_sync_len);
        frame.emplace_back(static_cast<float>(std::cos(phase)),
